...
class PrinterClient {
  ...
  async printTicketSystem(job) {
    ...
    try {
      ...
      let ticketsHtml = '';
      if (ticketData.type === 'checkout') {
        ticketsHtml = ticketData.tickets.map((ticket, index) => `
          <div class="ticket-item">
            <strong>Ticket ${index + 1}:</strong> ${ticket.title}<br>
            <strong>Evento:</strong> ${ticket.post.title}<br>
            <strong>Lugar:</strong> ${ticket.post.place}<br>
        ...
        ticketsHtml = `
          <div class="ticket-item">
            <strong>Ticket:</strong> ${ticketData.title}<br>
            <strong>Evento:</strong> ${ticketData.post.title}<br>
            <strong>Lugar:</strong> ${ticketData.post.place}<br>
            <strong>Precio:</strong> $${ticketData.price}
          </div>
        `;
      }
      ...
      const html = `
        ...
        <head>
          <meta charset="utf-8">
          <title>Ticket VentApp</title>
          <style>
            body { font-family: Arial, sans-serif; margin: 20px; }
            .ticket { max-width: 400px; margin: 0 auto; }
            .header { text-align: center; border-bottom: 2px solid #000; padding-bottom: 10px; }
            .content { margin: 20px 0; }
            .ticket-item { margin: 10px 0; padding: 10px; background: #f5f5f5; }
            .qr-code { text-align: center; margin: 20px 0; }
            .footer { text-align: center; border-top: 1px solid #000; padding-top: 10px; font-size: 12px; }
            @media print {
              body { margin: 0; }
              .ticket { margin: 0; }
            }
        ...
        <body>
          <div class="ticket">
            <div class="header">
              <h2>VENTAPP - TICKET</h2>
            </div>
            <div class="content">
              <p><strong>Cliente:</strong> ${ticketData.fullName || 'N/A'}</p>
              <p><strong>Email:</strong> ${ticketData.email || 'N/A'}</p>
              <p><strong>Teléfono:</strong> ${ticketData.phone || 'N/A'}</p>
              <p><strong>DNI:</strong> ${ticketData.dniPassport || 'N/A'}</p>
              ${ticketsHtml}
              ${ticketData.type === 'checkout' ? `<p><strong>Total:</strong> $${ticketData.totalPrice}</p>` : ''}
            </div>
            <div class="qr-code">
              <img src="${qrDataURL}" alt="QR Code">
              <p>${qrCode}</p>
